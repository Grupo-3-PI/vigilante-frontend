...
function carregarRodapePagina() {
    var data = new Date();
    document.getElementById("rodape-pagina").textContent = `Vigilante - gerado em ${data.getDate()}/${data.getMonth()}/${data.getFullYear()} ${data.getHours()}:${data.getMinutes()}:${data.getSeconds()}`
}
...
async function cadastrarRelatorio(titulo_relatorio, consulta, fk_Usuario) {
...
window.gerarRelatorio = async function () {
    ...
    cadastrarRelatorio("Relatório de criminalidade", consulta, sessionStorage.ID_USUARIO);
}
...
window.carregarTudo = async function (id) {
    //var jsonFinal = await pegarDadosDashboard()
    var jsonFinal = await pegarDadosRelatorio(id)
...
function carregarPercentualCrimes() {
    ...
    var filtro = "nulo";
    fetch(`/dashboard/percentualCrimes/${ano}/${filtro}`, {
        method: "GET",
        headers: {
            "Content-Type": "application/json"
        }
